...
| 维度 | CodeGraph | CodeSnap |
|------|-----------|----------|
| 架构 | MCP Server（常驻进程） | Skill + CLI（按需调用） |
| 启动方式 | `codegraph init -i` 等索引建完 | 即时可用，后台建索引 |
| 内存占用 | 100-300MB（常驻） | 0MB（空闲）/ 30-80MB（查询） |
| 二进制大小 | ~80MB（捆绑 Node.js） | **~5MB**（Rust musl 静态编译） |
| 搜索引擎 | SQLite FTS5（通用文本） | **三维倒排索引 + Trie**（代码定制） |
| 调用图 | SQLite 邻接表 JOIN | **CSR 压缩图**（L3 Cache 驻留） |
| 影响分析 | SQL 递归 CTE | **Roaring Bitmap 位运算** |
| 存储引擎 | SQLite B-Tree | **LSM Tree**（写优化 + 增量友好） |
| 索引新鲜度 | debounce 2s + 重索引 | **mtime 自检**，永远最新 |
| 跨会话持久化 | 内存缓存，重启需暖机 | **磁盘 mmap**，即开即用 |
| 配置 | 需编辑 MCP JSON | **零配置** |
| 适用场景 | 重度使用、跨工具 | **轻度使用、按需调用** |
